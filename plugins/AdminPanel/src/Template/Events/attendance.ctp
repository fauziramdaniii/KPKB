        ...
        <div class="col-lg-12">
            ...
            <div class="kt-portlet kt-portlet--height-fluid">
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="kt_datatable" id="table-events-attendances"></div>
                    <!--end: Datatable -->
...
<script>
    ...
    function delete_data(id) {
        $.post( "<?= $this->Url->build(['action' => 'deleteCategories']); ?>/" + id, { _csrfToken: '<?= $this->request->getParam('_csrfToken'); ?>' } )
            .done(function( data ) {
    ...
    var DatatableRemoteAjaxDemo = function() {
        var demo = function() {
            var datatable = $('#table-events-attendances').KTDatatable({
                data: {
                    type: 'remote',
                    source: {
                        read: {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        title: '<?= __('Start');?>',
                        template: function(row) {
                            return moment(row.start).format('YYYY-MM-DD h:mm:ss');
                        }
                    },
                    {
                        field: 'end',
                        title: '<?= __('End');?>',
                        template: function(row) {
                            return moment(row.end).format('YYYY-MM-DD h:mm:ss');
                        }
                    },
                    {
                        field: 'location',
                    ...
                    /** Action button **/
                    {
                        ...
                        template: function (row, index, datatable) {
                            return '<a href="<?= $this->Url->build(['action' => 'participants']); ?>/'+ row.id +'"class="btn btn-sm btn-clean btn-icon" title="Participants"><i class="la la-user"></i></a>';
                        }
                    }
                ]
            });
