            ...
            <div class="kt-portlet kt-portlet--height-fluid">
                <div class="kt-portlet__head kt-portlet__head--lg">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title"><?= __('Jadwal Pertandingan') ?></h3>
                    </div>
                    <div class="kt-portlet__head-toolbar">
                        <div class="kt-portlet__head-wrapper">
                            <a href="<?= $this->Url->build(['action' => 'addSchedule', $games_id]); ?>" class="btn btn-default btn-bold btn-upper btn-font-sm">
                                <i class="flaticon2-add-1"></i>
                                <?= __('Tambah Jadwal Pertandingan') ?>
                            </a>
                        </div>
                    </div>
                </div>
                <div class="kt-portlet__body">
                    ...
                    <div  class="table-responsive">
                        <div class="kt_datatable" id="table-schedules"></div>
                    </div>
                    <!--end: Datatable -->
...
<script>
    ...
    function delete_data(id) {
        $.post( "<?= $this->Url->build(['action' => 'deleteSchedules']); ?>/" + id, { _csrfToken: '<?= $this->request->getParam('_csrfToken'); ?>' } )
            .done(function( data ) {
    ...
    var DatatableRemoteAjaxDemo = function() {
        var demo = function() {
            var datatable = $('#table-schedules').KTDatatable({
                data: {
                    type: 'remote',
                    source: {
                        read: {
                ...
                columns: [
                    ...
                    {
                        field: 'MatchSchedules.match_time',
                        title: 'Waktu Pertandingan',
                        template: function(row) {
                            return moment(row.match_time).format('YYYY-MM-DD h:mm:ss');
                        }
                    },
                    {
                        field: "Actions",
                        ...
                        overflow: 'visible',
                        template: function (row, index, datatable) {
                            return '<a href="<?= $this->Url->build(['action' => 'detailSchedule']); ?>/'+ row.id +'"class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Rincian Pertandingan"><i class="la la-search"></i></a><a href="<?= $this->Url->build(['action' => 'editSchedule']); ?>/'+ row.id +'"class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Edit"><i class="la la-edit"></i></a><a href="javascript:delete_data('+row.id+');" onclick="return confirm(\'Are you sure delete #'+row.id+'\');" class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Delete"><i class="la la-trash"></i></a>';
                        }
                    }
