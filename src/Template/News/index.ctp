                    ...
                    <div class="post-item">
                        <div class="post-item-wrap">
                            <div class="post-image">
                                ...
                                <?php endif; ?>
                                <span class="post-meta-category"><?php foreach($news->tags as $tag) : ?><a
                                        href="#"><?= $tag->name; ?></a><?php endforeach; ?></span>
                            </div>
                            <div class="post-item-description">
                                <span class="post-meta-date"><i
                                        class="fa fa-calendar-o"></i><?= $news->created->format('d M Y'); ?></span>
                                <h2><a
                                        href="<?= $this->Url->build(['controller' => 'News', 'action' => 'view', $news->get('slug')]); ?>"><?= __($news->get('title')); ?>
                                    </a></h2>
                                <p><?= $this->Text->truncate(strip_tags($news->get('content')), 200); ?></p>
                                <a href="<?= $this->Url->build(['controller' => 'News', 'action' => 'view', $news->get('slug')]); ?>"
                                    class="item-link">Baca Selengkapnya <i class="icon-chevron-right"></i></a>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
                    ...
                    <div class="post-thumbnail-list">
                        ...
                        <div class="post-thumbnail-entry">
                            ...
                            <div class="post-thumbnail-content">
                                <a
                                    href="<?= $this->Url->build(['controller' => 'News', 'action' => 'view', $other_blog->get('slug')]); ?>"><?= __($other_blog->title); ?></a>
                                <span class="post-date"><i class="icon-clock"></i>
                                    <?= $other_blog->created->format('d M Y'); ?></span>
                            </div>
                        </div>
                        <?php endforeach; ?>
                        <?php else : ?>
